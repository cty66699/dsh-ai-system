...
const PATTERNS = [
  ...
  // ★★★ 2026-09-26 修（第 57 轮，**它报的第一件事又是它自己的 bug** —— 这已经是第三次了）：
  //   原版正则把 `git@ssh.github.com` 当成邮箱 —— 而那是 **ssh 的登录名约定**（`git@host`），
  //   不是邮箱。而这条检查**本来就是给"要公开出去的仓库"用的**，
  //   而那种仓库里**几乎必然**出现 `git@github.com`（remote URL、clone 命令、文档里的例子）。
  //   ⇒ 不修的话：**它会对着每一份正常文档报"邮箱泄露"** —— 而**一个会误报的检查会被无视**。
  //   ⇒ 修法：**排除本地部分为 `git`（ssh 约定）与 `noreply`（GitHub 的匿名邮箱）的那些**，
  //     并要求本地部分至少 2 个字符（`a@b.c` 那种噪声不值得报）。
  ['邮箱', /(?<![A-Za-z0-9._%+-])(?!git@|noreply@)[A-Za-z0-9._%+-]{2,}@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/],
