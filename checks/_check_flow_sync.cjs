...
for (const [text, label] of numDocs) {
  ...
  const PATTERNS = [
    /([一二三四五六七八九十]{1,3})项机械检查/g,            // 「十七项机械检查」—— 唯一无歧义的总结句
    /^#{1,6}\s*([一二三四五六七八九十]{1,3})项断言/mg,      // 标题行「## 十七项断言分别管什么」
    /这([一二三四五六七八九十]{1,3})项全绿/g,               // 「这十七项全绿」
    /统一检查入口[·\s]*([一二三四五六七八九十]{1,3})项断言/g, // 流程图节点「统一检查入口 · 十七项断言」
    /←\s*([一二三四五六七八九十]{1,3})项断言的脚本/g,        // 目录树注释
    // ★ 2026-09-26 补：**阿拉伯数字**也要认 —— 实测漏网一处：
    //   `AGENTS.md` 的「③ 统一入口（**17 项**；只有它绿了才算改完）」写的是**阿拉伯数字**，
    //   上面那 5 条（只认中文数字）全部漏过它 ⇒ 改完项数后这句**留在文档里没被发现**。
    //   这正是"断言自己覆盖不全"的典型 —— 补它，而不是每次手工搜。
    //   ★ 仍然只认**紧跟特定词**的形态（不认裸 `N 项`）：实测「13 项一律显示「⚠️ 有发现」」
    //     是**描述历史**、不是总数，用宽形态会误报。
    /统一入口[（(]\s*(\d{1,2})\s*项/g,                    // 「统一入口（17 项」
    /(\d{1,2})\s*项机械检查/g,                             // 「18 项机械检查」
    /统一检查入口[·\s]*(\d{1,2})\s*项断言/g,               // 流程图节点（阿拉伯）
    /←\s*(\d{1,2})\s*项断言的脚本/g,                       // 目录树注释（阿拉伯）
    // ★★ 2026-09-26 补（**第 5 次同型错的下半场**）：上面 8 条收得很紧（为防误报），
    //   于是漏了两处**也是总数语义**的写法 —— 它们出现在流程文档里：
    //     ·「一条命令跑完**全部断言**。**十七项**：」（后面接一张表）
    //     ·「**第 7 步一条命令跑完十七项**」
    //   ⇒ 两处都写"十七项"，而实际已 **18** 项；**①b 一条都没抓到**。
    //   ★ 是**新加的 ①c**（数"名字列表长度"）把这两处带出来的 —— 两条检查互补：
    //     ①b 数数字、①c 数名字；这次是"数字错了、而且名字也没列" ⇒ ①c 先报出来，顺藤摸到 ①b 漏了。
    //   ★ 为什么这两条形态可以收进来（不会像第一版那样误报）：
    //     「一条命令跑完」后面跟的**必然是总数**（它描述的是那句命令的覆盖面）；
    //     「第 N 步一条命令跑完」同理。**它们不像裸 `N 项` 那样会被"其中/另外"污染。**
    /一条命令跑完[^。\n]{0,12}?([一二三四五六七八九十]{1,3}|\d{1,2})\s*项/g,
    // ★★ 2026-09-26 第 63 轮补：**"N 项断言各管什么"这种形态也是总数语义** ——
    //   实测漏网：README 里那句「机器生成的：**17 项断言各管什么**、有没有反例测试」（实际 24 项），
    //   **错了不知多久，而 ①b 一条都没抓到** —— 因为上面那 9 条都要求"紧跟特定词"，
    //   而「N 项断言」不在其中（`N项断言` 这个宽形态**第二版就被否掉了**，见上面的注释：
    //   它会误伤「有一次三项断言」「这一项会先拦住」）。
    //   ★★ 为什么这两条可以加：**"各 / 分别 / 都 / 全"是「遍指词」** ——
    //     它们**只会在描述"一个集合的全部"时出现**；而当年那两处误报都不带遍指词。
    //     ⇒ 判据不是「N 项断言」这个词，而是**「N 项断言」后面跟了遍指词**。
    //   ★ 这正是"用什么当信号"那件事的又一次应用：**信号要选"只可能因为那件事出现"的那个。**
    //   ★★ 而它第一版**还是误报了一处**（双向验证抓到的）：`其中 7 项断言都通过` ——
    //     "其中"是**部分语义**，而"都"在那个局部里也成立 ⇒ 光靠遍指词不够。
    //     ⇒ 再加**负向断言**：前面不能是「其中 / 另外 / 这 / 那」。
    /(?<!其中\s?)(?<!其中)(?<!另外\s?)(?<!这)(?<!那)(\d{1,2}|[一二三四五六七八九十]{1,3})\s*项断言[^。\n]{0,6}?(各|分别|都|全)/g,
    /(?<!其中\s?)(?<!其中)(?<!另外\s?)(?<!这)(?<!那)(\d{1,2}|[一二三四五六七八九十]{1,3})\s*项机械检查[^。\n]{0,6}?(各|分别|都|全)/g,
  ]
