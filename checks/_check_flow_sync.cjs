...
// 公开 README 的**源**（`_public_src/README.md`）—— 访客看到的那份，最容易忘
const PUB_README = path.join(HERE, '_public_src', 'README.md')
if (fs.existsSync(PUB_README)) numDocs.push([readText(PUB_README), '_public_src/README.md（公开 README 源）'])
const CN_NUM = { 十一: 11, 十二: 12, 十三: 13, 十四: 14, 十五: 15, 十六: 16, 十七: 17, 十八: 18, 十九: 19, 二十: 20 }
...
for (const [text, label] of numDocs) {
  ...
  //   第一版写成 `([一二三四五六七八九十]{1,2})项`，于是把
  //   「另外 **9** 项在示例里显示 ⏭️」「只配置了其中 **7** 项」「**5** 项是…**3** 项是…」
  //   一并当成总数 ⇒ README 一处就报 5 个假阳性。
  //   **误报比漏报更糟**（会让人不再信任这个检查）⇒ 只匹配紧跟这些词的形态：
  const PATTERNS = [
    /([一二三四五六七八九十]{1,2})项机械检查/g,            // 「十七项机械检查」—— 唯一无歧义的总结句
    /^#{1,6}\s*([一二三四五六七八九十]{1,2})项断言/mg,      // 标题行「## 十七项断言分别管什么」
    /这([一二三四五六七八九十]{1,2})项全绿/g,               // 「这十七项全绿」
    /统一检查入口[·\s]*([一二三四五六七八九十]{1,2})项断言/g, // 流程图节点「统一检查入口 · 十七项断言」
    /←\s*([一二三四五六七八九十]{1,2})项断言的脚本/g,        // 目录树注释
  ]
...
const anyClaim = numDocs.some(([t]) => /[一二三四五六七八九十]{1,2}项机械检查|这[一二三四五六七八九十]{1,2}项|共\s*[一二三四五六七八九十]{1,2}项/.test(t))
if (!anyClaim) {
