...
for (const [text, label] of numDocs) {
  ...
  // ★★ 2026-09-26 第 70 轮修（**新数据照出的旧判据缺陷**，第二次）：
  //   原来是 `一条命令跑完\s*([^）\n]{0,600}?)\s*断言` —— **惰性匹配到第一个"断言"**。
  //   而第 70 轮新加的那一项**名字里就含"断言"**（「只读断言」）⇒
  //   实测：raw 变成 `…访客视角、只读` —— **"只读断言"被切成"只读"、而"快照"整个丢了** ⇒ 名字数少 1。
  //   ⇒ 判据收紧为：**"断言"后面必须紧跟右括号**（那句的形态是「…快照**二十五项**断言）」）。
  //   这样「一条命令跑完全部断言。」那类总述（后面是句号）仍然被跳过 —— 那条边界保持不变。
