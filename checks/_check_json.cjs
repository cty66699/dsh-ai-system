...
if (bad.length) {
  ...
  for (const b of bad) {
    ...
    if (b.ctx) {
      ...
      // 最常见的成因直接点出来，省一轮往返
      // ★★ 2026-09-26 修（独立核验「附带」条）：原来这个字符类里放的是**两个 ASCII 双引号**（码点 34,34），不是 U+201C/U+201D ⇒ **任何** JSON 报错（只要 ±70 字符上下文里有个 "）都会附上这句「直角双引号」的误诊。实测一个尾逗号错误也报了它。
      //   ⇒ 按本仓库自己的话：「一个诊断错的断言比没有断言更糟」。改成**真正的弯引号**。
