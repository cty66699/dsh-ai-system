...
module.exports = {
  ...
  JS_EXT: ['.cjs', '.js', '.mjs'],
  // 「访客第一步」那条命令 —— **唯一真源**（2026-09-26 第 56 轮立）。
  // 为什么它必须是真源：**它是「访客照着 README 敲的那一条」**，而它在两处被用 ——
  //   ① 生成器的「干净环境验证」（把整个仓库拷到临时目录后跑它）；
  //   ② `_check_remote_visitor.cjs`（从远端 clone 一份后跑它）。
  // ⇒ 两处**必须逐字一致**，否则会出现「本机验的是 A、访客跑的是 B」。
  //   ★ 是「重复清单」那道检查把这两处揪出来的（实测：它们当时已经一致，而检查的价值在「将来不一致时有人知道」）。
  VISITOR_CMD: ['checks/_check_all.cjs', '--fast', '--target', 'example/_target.json'],
}
