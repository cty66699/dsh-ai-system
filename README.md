...
> 真正的门槛是**磁盘约 1.8 GB** 与**能连上 npm**，**不是硬件性能**。
>
> | | 要求 |
> |---|---|
...
> **先说清楚这是什么（以及不是什么）**
>
> **这是**：一套**断言层 + 治理规则**——**十七项**机械检查（引用、计数、逐条处置、跨轮欠账、文档漂移、占位符、文件卫生、配置存活……），
...
|---|---|---|
| **是什么** | 断言层 + 治理规则：十七项机械检查、以及"谁能喊停"的权力规则 | 一个能跑起来的 AI 宿主：跨会话记忆、多智能体、用量账本、Web UI |
| **依赖** | **零依赖**。对**任何**项目都能用 —— 它检查的是你的产物 | 需要 **Windows + PowerShell 5.1+ + Node ≥ 22.19 + pnpm**；会装 7 个插件（约 1.8 GB 磁盘）|
...
**15 种占位符里哪些 AI 能自己填、哪些必须问你** · **7 条红线**（不许跳验证、不许擅自 `-Force`、不许把猜测写成事实……）·
...
| **磁盘** | **约 1.8 GB** | DSH 本体 462 MB + profile 365 MB + pnpm 共享缓存 945 MB |
...
> 真正的门槛是**磁盘（约 1.8 GB）**与**能连上 npm**，不是硬件性能。
...
> **⚠️ 那 9 个「跳过」不是同一回事 —— 输出里可以分清，但一眼扫过去容易混。** 三种情况：
>
> | 情况 | 例子 | 含义 |
> |---|---|---|
...
> 示例工程只配置了其中 8 项能跑的对象；**另外 9 项在示例里显示 ⏭️ 并写明原因** ——
...
> ```
> （按需运行 —— 它要往临时目录写缺陷样本，所以不在上面那条命令里。实测 5 / 5 全部"咬到了"。）
>
...
- ❌ **一份材料可以在这十七项全绿的情况下依然没答对题** —— 这是最根本的已知盲区；
...
1. **上面那个「8 / 8」只演练了十七项里的七项。** 另外九项显示 ⏭️ —— 但**它们是三种不同的原因**：**五项是"示例没给它们配对象"**（你的项目配了就会跑）、**三项是"本子集没含实现"**（依赖作者本机运行时，占位跳过）、**一项是你主动关掉的**（`--fast` 或 `budgetLedger: null`）。**别把三种原因混成一个「跳过」** —— 第一种是你的责任，第二种是这一版的边界，第三种是你自己的选择。**「跳过」不等于「通过」**，只是这一份演示里它们没跑。
...
LICENSE            ← MIT
docs/              ← 文档集（见下表）
checks/            ← 十七项断言的脚本 + 路径解析（_paths.cjs 是配置层）
