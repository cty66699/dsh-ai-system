...
> | **磁盘** | **约 2.6 GB**（随版本与插件变；[`REQUIREMENTS.md`](REQUIREMENTS.md) 里给了量的脚本）|
...
> **先说清楚这是什么（以及不是什么）**
>
> **这是**：一套**断言层 + 治理规则**——**二十五项**机械检查（引用、计数、逐条处置、跨轮欠账、文档漂移、占位符、文件卫生、配置存活……），
...
|---|---|---|
| **是什么** | 断言层 + 治理规则：二十五项机械检查、以及"谁能喊停"的权力规则 | 一个能跑起来的 AI 宿主：跨会话记忆、多智能体、用量账本、Web UI |
...
**[`AI-INSTALL.md`](AI-INSTALL.md) 里写了**：环境自检命令 · 退出码语义（0/1/2 各代表什么）·
...
> 照代码块复制的人会**直接真装**（[`AI-INSTALL.md`](AI-INSTALL.md) 的红线 4 明写「**不许在 `-DryRun` 没看过的情况下直接真装**」）。
...
| **磁盘** | **约 2.6 GB**（随版本与插件变；[`REQUIREMENTS.md`](REQUIREMENTS.md) 里给了量的脚本）| 全局安装 0.37 GB + `DSH_HOME` 1.34 GB + pnpm 共享缓存 0.92 GB |
...
- ❌ **一份材料可以在这二十五项全绿的情况下依然没答对题** —— 这是最根本的已知盲区；
...
## ★ 这套东西**没有在哪些条件下验过**（2026-09-26 补）

> **为什么单列一节**：上面「不保证」说的是「**它做不到什么**」，而这一节说的是「**我没验过什么**」——
> **后者更容易被忽略**，而它对使用者同样重要。**没验过的，就写"没验过"，不靠"我猜没问题"糊过去。**

| 没验过的条件 | 我实际做到哪一步 | 你该怎么对待它 |
|---|---|---|
| **一台真正干净的电脑** | 只用过「**干净目录 + 影子 `DSH_HOME`**」—— 而那是**同一台机器、同一个 Node、同一套已装好的前置** | 「装得起来」这件事，**没有被一个真正陌生的环境压过**。装的过程里对"机器现状"的假设（Node 在哪、npm registry 通不通、有没有代理）**都没被反向验证** |
| **Linux / macOS** | 只把 `process.platform` **伪装成 `linux`** 跑了一遍断言层（读数见上面那一节） | **断言层大概率能跑**（全仓 0 处硬编码 Windows 路径）—— 而**部署脚本根本没有对应实现**，得照那十步手工做 |
| **PowerShell 7（`pwsh`）** | 本机只有 **PowerShell 5.1**；脚本声明的门槛也是 **5.1+** | 5.1 上**每次都真跑过**；**`pwsh` 上没人验过** —— 两者有个别语法差异（例如 `??`、`\|\|`），**不保证** |
| ~~GitHub 网页端的渲染~~ | ✅ **已验**（2026-09-26）：`mermaid.parse()` 通过 + `mermaid.render()` 产出 SVG + GitHub 的 `data-type="mermaid"` 容器 + 页面里 0 处渲染错误标记 | **高置信度能画出来** —— 而**「亲眼看到 GitHub 画出它」这一条我没做到**（headless 下它的异步渲染没触发，如实标注） |

**⇒ 这三条不是"以后再补"，而是"在当前条件下补不了"**（需要**另一台机器 / 另一个平台 / 另一个 shell**）。
**⇒ 所以它们的正确写法就是**「未验证」** —— 而这条界线，请你自己划。
**另外三件同量级的事实，也说在这里（不藏在运行时输出的小字里）：**

1. **上面那个「10 / 10」只演练了二十五项里的十二项。** 另外八项显示 ⏭️ —— 但**它们是三种不同的原因**：**五项是"示例没给它们配对象"**（你的项目配了就会跑）、**三项是"本子集没含实现"**（依赖作者本机的运行时）、**还有一项是你主动关的**（预算闸门，配置里写成 `budgetLedger: null`）
...
| **要接进自己的项目** | **[`配置说明.md`](docs/配置说明.md)**（字段总表 + 最小可用配置 + 退出码）· **[`产物格式.md`](docs/产物格式.md)**（那几份目标文件长什么样）|
| **想知道"这套东西是怎么做出来的、踩过什么"** | **[`心得.md`](心得.md)** —— 七条改掉的想法，每条附它产生的现场 |
| **想改路由 / 看真实样例** | **[`路由策略示例.md`](docs/路由策略示例.md)** |
| **开始往外派活、想要规矩** | **[`派发流程.md`](docs/派发流程.md)**（八步）· **[`治理规则.md`](docs/治理规则.md)**（谁能喊停、发现必须逐条处置）|
| **已经在跑核验、要给核验者写任务书** | **[`T3-规格符合性.md`](docs/T3-规格符合性.md)** · **[`T4-一致性对账.md`](docs/T4-一致性对账.md)** · **[`T5-代码核验.md`](docs/T5-代码核验.md)** · **[`T6-裁定复核.md`](docs/T6-裁定复核.md)** · **[`T7-对抗性推演.md`](docs/T7-对抗性推演.md)**（五条轨道，各管一类问题）|
| **想知道"该信哪个核验者的哪类结论"** | **[`核验者能力档案.md`](docs/核验者能力档案.md)** |

> **★ 一句话**：**这些不是"必读清单"，是"按需查阅"** ——
...
LICENSE            ← MIT
docs/              ← 文档集（见下表）
checks/            ← 二十五项断言的脚本 + 路径解析（_paths.cjs 是配置层）
...
| 文件 | 什么时候看它 |
|---|---|
| ★ **[`配置说明.md`](docs/配置说明.md)** | **动手配 `_target.json` 之前** —— 每个字段干什么、**5 个示例里从没露面的启用键**、"不配置"的三种写法、退出码 |
| ★ **[`保障清单.md`](docs/保障清单.md)** | **想知道"那个通过数有多少含金量"时** —— **机器生成**的，五节：① 统一入口里**每一项**断言各管什么、有没有反例测试；② 生成公开集时的**几道闸**、哪一道是**预防性**的；③ 反例自检（"能不能咬人"）；④ **同一类错在不同文件类型上的拦截厚度**；⑤ **你实际敲的那两个入口各自守着什么** |
| ★ **[`产物格式.md`](docs/产物格式.md)** | **动手写那七份产物之前** —— 数据流、**哪两处必须逐字对得上**、四个真会卡住的地方 |
| [`治理规则.md`](docs/治理规则.md) | 想知道"这些规则**为什么**存在" —— 每条都附了它产生时踩的坑 |
| [`派发流程.md`](docs/派发流程.md) | 想复刻"把产物送去独立核验、再逐条裁定"这套流程 |
| [`核验者能力档案.md`](docs/核验者能力档案.md) | 想知道"哪种模型适合查哪一类问题" |
| `T3`–`T7` 任务书 | 每次派发核验时用的模板（**本子集只含 T3–T7**；T1/T2 的规程写在一个未发布的技能包里） |
| [`路由策略示例.md`](docs/路由策略示例.md) | 作者自己的模型路由样例 —— **是给人读的样例，不是给 agent 吃的策略** |

> **为什么这里没有 `AGENTS.md`**：那份文件原本会被 DSH 类工具**自动注入每一个会话的系统提示**。
> 把它放在仓库根，等于让**访客的 agent** 也吃下作者的私有策略（含派发、预算、路由）。
> ⇒ 它被改名成 [`docs/路由策略示例.md`](docs/路由策略示例.md)：**是给人读的样例，不是给 agent 吃的策略。**
...
|---|---|---|---|
| **目标声明**（[`example/目标声明.md`](example/目标声明.md)） | 目录符合性 | `declaration` | 该项跳过 |
| **成品**（[`example/成品.md`](example/成品.md)） | 目录符合性 / 占位符 | `product` | 该项跳过 |
| **符合性映射**（`example/符合性映射.json`） | 目录符合性 | `conformanceMap` | 该项跳过 |
| **核验报告**（[`example/核验报告.md`](example/核验报告.md)） | 发现处置链路 | 由 `dispositionScanDirs` 扫到 | 该项跳过 |
| **裁定记录**（[`example/裁定记录.md`](example/裁定记录.md)） | 发现处置链路 | 同上 | 该项跳过 |
...
> **[`docs/派发流程.md`](docs/派发流程.md) 里的代码片段不是通用 API** —— 它是**这一套运行时**的写法。
