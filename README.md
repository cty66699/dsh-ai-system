...
> **先说清楚这是什么（以及不是什么）**
>
> **这是**：一套**断言层 + 治理规则**——**十六项**机械检查（引用、计数、逐条处置、跨轮欠账、文档漂移、占位符、文件卫生、配置存活……），
...
|---|---|---|
| **是什么** | 断言层 + 治理规则：十六项机械检查、以及"谁能喊停"的权力规则 | 一个能跑起来的 AI 宿主：跨会话记忆、多智能体、用量账本、Web UI |
...
> 在本子集里那一项是**占位实现、本来就跳过**，所以**带不带它结果完全一样**（都 7 / 7、都 exit 0）——
> 实测确认过，不是推测。
> **在你自己的环境里**：如果你配好了那一项、又不想每次收尾都跑它，**那时**才需要 `--fast`。

**`--target` 也可以省** —— 不给参数时自动使用 `example/_target.json`，所以下面这条**结果相同（7 / 7）**：
...
> 示例工程只配置了其中 7 项能跑的对象；**另外 9 项在示例里显示 ⏭️ 并写明原因** ——
...
> ```
> （按需运行 —— 它要往临时目录写缺陷样本，所以不在上面那条命令里。实测 4 / 4 全部"咬到了"。）
>
...
- ❌ **一份材料可以在这十六项全绿的情况下依然没答对题** —— 这是最根本的已知盲区；
...
1. **上面那个「7 / 7」只演练了十六项里的七项。** 另外九项显示 ⏭️ —— 但**它们是三种不同的原因**：**五项是"示例没给它们配对象"**（你的项目配了就会跑）、**三项是"本子集没含实现"**（依赖作者本机运行时，占位跳过）、**一项是你主动关掉的**（`--fast` 或 `budgetLedger: null`）。**别把三种原因混成一个「跳过」** —— 第一种是你的责任，第二种是这一版的边界，第三种是你自己的选择。**「跳过」不等于「通过」**，只是这一份演示里它们没跑。
...
LICENSE            ← MIT
docs/              ← 文档集（见下表）
checks/            ← 十六项断言的脚本 + 路径解析（_paths.cjs 是配置层）
